...
function Handle_Operator(Next_Operator) {
    ...
    // When an aperator key is pressed we convert the current number displayed on the screen to a number and then store the result in Calculator.First_Operand if it doesn't already exist.
    const Value_of_Input = parseFloat(Display_Value);
    // Checks if an operator already exists and if Wait_Second_Operand is true, then
    // updates the operator an exits from the function.
...
keys.addEventListener('click', (event) => {
    // The target variable is an object thtat represents the element that was clicked.
    const { target } = event;
